#!/usr/bin/env python3
"""Static site generator for ppcconsultancy.uk (UK sister site of googleadsfreelancer.com).
...
BUDGET_BANDS = [("under_2k", "Under £2K / month"), ("2k_15k", "£2K to £15K"), ("15k_40k", "£15K to £40K"), ("40k_plus", "£40K+")]
...
PROOF_DEFAULT = [("14+", "years in paid media"), ("1", "consultant on your account"), ("Flat fee", "never a % of ad spend"), ("Yours", "accounts, tags and pages stay with you")]
...
# Consent Mode v2: everything denied until the visitor answers the banner (PECR / UK GDPR). Runs before GTM.
CONSENT_JS = ("<script>window.dataLayer=window.dataLayer||[];function gtag(){dataLayer.push(arguments)}"
              "(function(){var c=null;try{c=localStorage.getItem('pc_consent')}catch(e){}"
              "var g=c==='granted'?'granted':'denied';"
              "gtag('consent','default',{ad_storage:g,ad_user_data:g,ad_personalization:g,analytics_storage:g,functionality_storage:'granted',security_storage:'granted',wait_for_update:500});"
              "window.__pcConsent=c;})();</script>")
CONSENT_BANNER = ('<div class="consent" id="consent" role="dialog" aria-live="polite" aria-label="Cookie choices" hidden>'
                  '<p>I use Google Analytics and Google Ads measurement to see which pages bring enquiries. Nothing is set until you choose. '
                  ...
                  "<script>(function(){var b=document.getElementById('consent');if(!b)return;if(window.__pcConsent!=='granted'&&window.__pcConsent!=='denied')b.hidden=false;"
                  "b.addEventListener('click',function(e){var t=e.target.closest('button[data-c]');if(!t)return;var v=t.getAttribute('data-c');"
                  "try{localStorage.setItem('pc_consent',v)}catch(x){}"
...
def hreflang_html(slug):
    ...
    return (f'<link rel="alternate" hreflang="en-GB" href="{here}">'
            f'<link rel="alternate" hreflang="en-US" href="{SISTER}{other}">'
            f'<link rel="alternate" hreflang="x-default" href="{SISTER}{other}">')
...
<a class="logo" href="/">ppc<span>consultancy</span>.uk</a>
...
<p>This site is a set of static pages. Measurement runs through Google Tag Manager and only stores anything after you accept the cookie banner. If you reject, the tags run in cookieless mode and no identifier is stored on your device.</p>
<ul><li><b>Google Analytics 4.</b> Measures visits, pages viewed, clicks on the quote and email links and form submissions, to understand which pages are useful. With consent it sets first-party cookies (<code>_ga</code>, <code>_ga_*</code>). Data is processed by Google under <a href="https://policies.google.com/privacy" rel="noopener">Google's privacy policy</a>.</li>
<li><b>Google Ads conversion measurement.</b> When you arrive from a Google ad and send the form, the enquiry is reported back to Google Ads as a conversion so ad spend can be judged on real outcomes. Also subject to your consent choice.</li></ul>
<p>You can change your choice at any time by clearing this site's data in your browser; the banner will ask again. The site works the same either way.</p>
...
<p>Want to add anything? Write to {email_link()}, and include read-only access to Google Ads and GA4 if you already know you want an audit.</p>
    ...
    return {"slug": "thanks", "short": "Thanks", "blurb": "", "title": "Message received | PPC Consultancy",
